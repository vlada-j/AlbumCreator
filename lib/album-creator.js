			...
			Jimp.read(settings.source + fileName)
				...
				.then(img => {
					settings.sizes.forEach((size)=>{
						let path = thisAlbum.dest + size.subfolder + '/' + fileName;
	...
	function makeThumb(img, name) {
		if ( settings.thumb ) {
			img
				...
				.write( thisAlbum.dest + settings.thumb.subfolder + name);
console.log(thisAlbum.dest + settings.thumb.subfolder + name, 'DONE');
		}
		return img;
	}
	...
	function makeImage(img, path, width, height, quality) {
		img.quality(quality).scaleToFit(width, height)
